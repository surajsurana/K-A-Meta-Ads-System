...
## System self-audit (added 2026-09-23, real user request — do this yourself, not a specialist dispatch, since it's about the system's own design, not ad performance)

Once a month, take a real look at whether the *system itself* is missing anything — not ad performance, but gaps in what gets checked, drift between what's documented and what actually runs, and structural blind spots. This step exists because a real one sat unflagged for months: international revenue existed with essentially zero acquisition spend behind it, and the standing geography check never caught it because it only ever checked one direction (spend without orders, never the reverse) — it took a direct, open-ended user question in an interactive session to surface it.

1. **Read the last month of `knowledge/learning-log.jsonl`** for any entry that reads like a gap being found reactively — a finding that only surfaced because the user asked a pointed question, an interactive-session fix to something the standing pipeline should have caught, or an agent explicitly noting it couldn't do something a headless run needs (e.g. "this only works interactively"). These are the strongest signal of where the standing checks are thin.
2. **Cross-check each `.claude/agents/*.md` file against `docs/architecture.md`** for drift — a policy documented in architecture.md that no agent's own checklist actually reflects, or an agent file that's grown stale against a policy that's since changed.
3. **For each real gap found this way, propose a specific, concrete fix** — the exact file and section that would change, and what the new text would say, not a vague "consider adding X." Log each as its own `type: decision` entry via `scripts/append-learning-log.sh`, and send it for approval via `scripts/send-telegram-approval.sh` exactly like any other decision plan — nothing gets edited until Suraj approves it. This process only ever proposes; it never edits an agent/prompt file itself.
4. **A quiet month with nothing found is a valid, expected outcome** — same "don't log what confirms nothing changed" principle as everywhere else in this project. Don't manufacture a finding just to have something to report.

This is explicitly not a request for general "how could this be better" brainstorming. Every proposal must trace to a real, specific incident or a real, specific documented-vs-actual mismatch found this cycle — a speculative improvement idea with no concrete evidence behind it doesn't clear the bar, same discipline as every other finding in this system. If a self-audit proposal reaches a ready plan, name it plainly in this run's notification (see below) alongside any other plan awaiting approval.
...
- **The monthly spend/orders/returns recap always gets a notification, every month, regardless of whether the strategic-intelligence side found anything (added 2026-08-23, user request)** — same "never silent about the numbers" standard as the weekly report. Use this exact structure (simple English, real line breaks, no jargon):
  ...
  <system self-audit finding, if any - what's missing and the proposed fix, plain English>
