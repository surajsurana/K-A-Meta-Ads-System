...
1. Dispatch **performance-analyst** for the full standing checklist per its own `.claude/agents/performance-analyst.md`: every active campaign, every active ad set, every active ad, fresh-pulled this cycle — spend, delivery, CPO, ROAS (Stitchflow alone, never summed with Shopify), conversion funnel, CTR, frequency, creative performance, Instagram follower count + delta, new-creative status, an explicit call-out for any ad set running on just one ad, **geographic and demographic performance** (`docs/architecture.md` §3c) — Stitchflow residence-based geography vs. Meta delivery by commercial outcome, DOB coverage % and what the known-DOB sample shows, Meta's own age-bracket performance stated separately from actual customer age — and **scale-readiness signals** (`docs/architecture.md` §3e) — headroom (frequency/CPM trend) on any strong-performing ad set, plus real Stitchflow order-history seasonality via `get_monthly_summary` (never `get_monthly_orders_report`, see §3e for why).
2. Dispatch **social-community-manager** (discovery half) for its content sweep — new K&A own-content, UGC, and Media Library uploads discovered since the last check (`.claude/agents/social-community-manager.md` "What you own" §3, added 2026-09-23).
3. Dispatch **campaign-strategist** for:
   ...
   - **The standing "should the ceiling go up" question** (`docs/architecture.md` §3e) — using this cycle's headroom/seasonality signals plus whether the trend has now held across several consecutive weekly checks (never act on a single strong week). If justified: a ≤20% ceiling step, or a small new-geography test budget, proposed with the same current/proposed/resulting/within-ceiling discipline as any allocation call. No sustained signal this cycle means no proposal, same as any other quiet-week outcome — this is a standing question to ask, not a standing bias toward scaling.
