...
- Send one consolidated notification (§8) at the end covering the week's findings, **using this exact structure every week, regardless of what else is or isn't noteworthy (added 2026-08-23, user request — simple English, real line breaks, no jargon, no wall of text):**
  ...
  **Non-India active ad sets get their own named line every week, never silently folded in (added 2026-08-31, real gap — USA and Canada ad sets were live and spending for weeks with zero mention in the digest).** The Spent/Orders/CPO/ROAS block above is Stitchflow-sourced and Stitchflow is India-centric, so it structurally cannot surface non-India activity — that block being "fine" says nothing about whether e.g. `USA Engaged 365D & Followers` or `Canada Engaged 365D & Followers` are healthy. Any ad set targeting outside India that is `effective_status: ACTIVE` this cycle must get one plain-English line in the digest every week (not just when something changed): name, weekly spend, Meta's own result metrics (messaging conversations started, link clicks, follower growth — whatever it's actually optimizing for, and say plainly if pixel-attributed purchases are zero rather than omitting the line), **and that country's actual Stitchflow order count/revenue for the period (§3c Geography — the spend-vs-orders check, not just Meta-side engagement)** — pull via `get_geographic_report` specifically, not `get_country_wise_report` (found unreliable/undercounting 2026-08-31, see §3c). Flag any single business/wholesale-looking account dominating that country's order count separately from genuine individual retail orders (§3c). This is a reporting completeness rule, not a new geographic strategy decision — no disposition call is implied just by including the line; a finding that implies one still routes to campaign-strategist as usual (§3c Ownership).
