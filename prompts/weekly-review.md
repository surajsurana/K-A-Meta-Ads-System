...
0. **Held-plan sweep (added 2026-08-24, user request) — do this first, yourself, before dispatching anyone.** Run `knowledge/RETRIEVAL.md` recipe 8 (due-now sweep) filtered to entries tagged `telegram-approval`/`held`. For each one whose `follow_up` date has arrived, check (same two-step pattern as any due-now item) whether a later `type:override` or `type:change` entry is `linked_to` that same plan id — if so, it's already been approved or rejected since, skip it. If it's genuinely still unresolved, call `scripts/send-telegram-approval.sh <plan-id>` to resend it with fresh buttons, and list it plainly in this week's digest under a **"Hold items awaiting your decision"** section (plain English, not just the id — what the plan actually does). If the user holds it again this week, that creates its own new observation entry with its own fresh 7-day `follow_up`, so the cycle continues automatically every week until they actually approve or reject it — nothing further needed from this run to keep that going.
...
- Send one consolidated notification (§8) at the end covering the week's findings, **using this exact structure every week, regardless of what else is or isn't noteworthy (added 2026-08-23, user request — simple English, real line breaks, no jargon, no wall of text):**
  ...
  <Hold items awaiting your decision - any plan resurfaced by step 0 above, plain English, one line each>
  ```
