...
2. Do a **targeted live anomaly pull** — not the full weekly sweep: account-level spend pacing (over/underspend vs. daily budgets), any active ad with `effective_status` showing `DISAPPROVED`/`WITH_ISSUES`, a quick frequency/CTR sanity check against recent baselines for active ad sets, and **account-level health** (added 2026-09-07, real precedent — the account was actually disabled once, 2026-08-31): `GET /act_686170454752172?fields=account_status,disable_reason` — `account_status` should read `1` (ACTIVE) and `disable_reason` should read `0` (none); anything else is a real, urgent finding (the account stops delivering entirely, unlike a single disapproved ad), escalate immediately regardless of how the rest of the pull looks. This was previously only checked weekly — a billing/account failure could otherwise sit uncaught for up to 6 days. Dispatch **performance-analyst** for this — do not pull the data yourself in this top-level context.
