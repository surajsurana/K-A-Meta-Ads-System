...
## Telegram-approval dispatch (added 2026-08-21, resolved — see `docs/architecture.md` §3 guardrail 1 resolution note)

If you are receiving this prompt from `scripts/telegram_approval_listener.py`: the plan's approval was verified in code, before this session was ever created, not asserted by any agent — Telegram's own callback signature ties the tap to a specific real account, and the listener independently checked that account against the one authorized `chat_id`, atomically claimed the plan, and confirmed it wasn't stale (>48h old) or already resolved. This is a second legitimate direct-approval channel per guardrail 1's resolution, not the relayed-agent-claim pattern that guardrail blocks. Proceed with the execution protocol above for the exact plan id given in this prompt: re-read it fresh, confirm it hasn't already been executed, redo its own freshness pre-check against live state, and only then execute verbatim, verify, and log — exactly as you would after an interactive approval. If the fresh pre-check fails, report `STALE_NOT_EXECUTED` and make no Meta/Instagram call; never execute anything beyond the exact endpoint this specific plan specifies.
...
## Building a video ad from an existing Instagram reel (added 2026-09-07, real incident)

**This works — never report "no way to pull the video" or treat it as needing an interactive-only tool.** A real dispatch did exactly that (reported the whole build as unbuildable) even though every step below uses nothing but the same `Bash`/`curl`/`meta_token.txt.txt` access already granted for every other Meta call, headless included. Independently re-verified live the same day and it worked on the first try. The method, given a reel's permalink shortcode (e.g. from `.../reel/DbcaqhZR8d8/`) or a known Instagram media id:

1. **Resolve a shortcode to a media id** (skip if the plan already gives you a numeric media id): `GET /{ig-user-id}/media?fields=id,permalink` (the account's `ig-user-id` is `17841401625784277`) and match by permalink. `ig-user-id` is not `page_id` — don't confuse the two.
2. **Get the downloadable video**: `GET /{media-id}?fields=media_url,thumbnail_url` — both are direct, temporary CDN URLs, no special permission beyond the standard token. `media_url` is the video file; `thumbnail_url` is a still frame, useful as the creative's `image_url` (Meta re-hosts and hashes it automatically on creative creation — no separate image upload step needed).
3. **Download, then upload as a new ad video asset**: `curl` the `media_url` to a local file, then `POST /act_.../advideos` with `-F "source=@<file>" -F "title=<internal label>"`. Returns a new `video_id`.
4. **Wait for processing before using it**: `GET /{video_id}?fields=status` until `status.video_status` is `"ready"` (starts `"processing"`) — building a creative against an unprocessed video will fail or behave unpredictably.
5. Build the creative/ad from there exactly as any other video ad (see below for the object_story_spec shape question).

**Before assuming an ad creative has a headline/body-text field, check a live reference ad in the same ad set — don't assume the copy brief's HEADLINE/PRIMARY TEXT necessarily has anywhere to go.** Not every ad pattern in this account carries stored text: some WhatsApp-destination `video_data` object_story_specs have no `title`/`message` field at all (confirmed live 2026-09-07 by inspecting two currently-active ads in ad set `6862975169176`) — those ads show only the video and a CTA button, nothing else; the customer never sees a separate headline or body copy, and the ad's own `name` field is purely an internal Ads-Manager label, never customer-facing. If that's the pattern, use the copy brief's headline as the new ad's internal `name` only, and don't try to force it into a field that doesn't exist in the proven-working shape. Always confirm by `GET`ting a currently-live ad's actual `object_story_spec` in the same ad set before building — never assume a field exists or is absent from a copy brief's own framing alone. (This exact question — does this ad set's pattern have a title/message field — was flagged as an open item twice, 2026-08-18 and 2026-09-07, without ever being permanently resolved; it's resolved now, but the general discipline — verify the live shape, don't assume — applies to any ad set, not just this one.)
