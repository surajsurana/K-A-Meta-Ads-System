...
5. **Preparing the post/reply plan, once triaged** — you do NOT call these endpoints; you specify exactly what marketing-lead should call:
   ...
   - **Requesting UGC permission via Instagram DM is NOT an automatable action (found live 2026-08-26) — Instagram's Messaging API does not allow a business to send the first message to someone who hasn't messaged the business already; this is a Meta platform restriction, not a missing capability to build.** When a disposition requires reaching out to a creator for permission, still draft the exact message text (that part is genuinely useful), but mark the plan explicitly **"MANUAL SEND REQUIRED — no automatable execution path"** and route it via a plain notification (`docs/proactive-operations.md` §8) with the ready-to-copy text, never through `scripts/send-telegram-approval.sh` — sending it with live Approve/Reject/Hold buttons implies tapping Approve will send it, and it structurally cannot (confirmed live: a user approved exactly this, and marketing-lead correctly reported FAILED rather than pretending to send it, but the confusing button-based framing was the actual gap). A public comment on the creator's own post, asking them to DM K&A first, is a workaround worth suggesting if reaching them matters enough to the user — that path *is* automatable via the comment-reply mechanism above, since it doesn't require initiating a DM.
