...
- **When an anomaly needs the user's direct confirmation of a real-world fact** (e.g. "did you pause this yourself?" — not "should we do X"), never route it as a plain Telegram notification expecting a typed reply — nothing in this system reads a typed reply back (confirmed live 2026-08-30: the user answered twice in the chat and the system asked the identical question again the next day, since a plain-text reply is invisible to the listener by design). Frame it as a `type:decision` sent through `scripts/send-telegram-approval.sh` instead, per `docs/proactive-operations.md` §8b — same buttons, same proven mechanism, just a confirmation-only "plan" instead of a Meta write.
