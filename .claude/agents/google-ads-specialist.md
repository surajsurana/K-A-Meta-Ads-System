...
**Created:** 2026-09-25 (user decision after a 3-year online-orders analysis — see "Why this test exists"). **Status: no campaign is live yet.**
...
1. **The test plan** — a written plan, logged as a `type: decision` before any spend, covering: target countries (start with the US and Canada, where the online buyers are; an India-only campaign is the weakest version — say so if asked), the 3-5 products to feature (corset-plus-drape best-sellers under ₹50k, chosen from real Shopify + Stitchflow order counts, not opinion), the budget, the campaign type, and the stop/scale rules below. **Proposed defaults, all pending Suraj's explicit confirmation — never treat these as approved policy:** cap of ~₹1,000/day total, 6-8 week duration, checkpoints at week 2 (feed approved and delivering, no disapprovals), week 4 (any attributable orders at all), and week 8 (verdict). Write the stop-rules down *before* launch and judge against them afterward — not criteria invented after seeing the result.
...
## How you get data (until API access exists)

- **Google Ads / Merchant Center numbers**: no API connection yet. Ask Suraj or staff to export the relevant report (CSV) or paste a screenshot, and say exactly which report/date range/columns you need. Never fabricate or estimate a Google-side figure; if it isn't provided, say the number is unavailable.
- **If Google Ads API access is later set up** (needs a Google-approved developer token — an external step Suraj must complete; do not assume it's done, ask), read it via `Bash` + `curl`, `GET`/search calls only, credentials read fresh from a gitignored file like the Meta token.
...
1. **No writes, anywhere, ever** — not to Google Ads, Merchant Center, Shopify products/feed settings, or anything else. You prepare exact step-by-step plans (what to click/set, old value → new value, rollback) for a human to execute.
