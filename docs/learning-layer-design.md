...
- **No hard expiry of the log itself — still true, updated 2026-09-05.** But "too much old stuff" became a real, measured problem sooner than this doc originally assumed: at 253 entries, the daily heartbeat's due-now sweep (recipe 8) — which used to re-scan the *entire* log from scratch every single day just to find which `follow_up`s were still open — was producing several hundred KB of grep output and enough real processing time that the heartbeat blew past its own 20-minute cap twice in one week (2026-09-02, 2026-09-05), even on days with nothing actually wrong. Fixed by adding `knowledge/open-followups.json`, a small always-current index of just the still-open follow-ups, maintained incrementally by `scripts/update-open-followups.py` on every write (see `scripts/append-learning-log.sh`) instead of re-derived from scratch daily — the *daily* cost is now constant regardless of how large the full log grows, which is the part that actually matters for the heartbeat. The log itself is still never deleted or archived by this fix — this only changes how the due-now sweep specifically reads it.
- **Rollup/archival for numeric performance history — proposed and built 2026-09-05.** Suraj's proposal, now live: weekly performance summaries (spend/orders/blended CPO/ROAS) roll up into monthly once a full calendar month's weeklies exist, monthly into quarterly once a full quarter's months exist, and so on through half-yearly/yearly — bounding total volume indefinitely (a 10-year-old account only ever holds ~9 yearly summaries plus the current year's finer-grained ones). This applies to **plain numeric aggregates only**, which compress with no real loss — it does *not* apply to the narrative learning-log entries above (decisions, experiments, overrides), where the specific reasoning is the entire point and a rolled-up summary would destroy exactly the detail this log exists to preserve; narrative entries are never touched by this and have no rollup/deletion mechanism at all.

  **Store:** `knowledge/performance-summaries.jsonl` — a separate file from the learning log itself (a structured numeric record, not a narrative one), one row per period, `granularity` one of `weekly`/`monthly`/`quarterly`/`half-yearly`/`yearly`. Before this existed, the weekly/monthly reviews' five headline numbers (`docs/proactive-operations.md` §8) were computed fresh every cycle and only ever sent as Telegram text — nowhere machine-readable, so there was nothing to roll up. Weekly/monthly reviews now also call `scripts/append-performance-summary.sh` (same fetch/rebase/append/commit/push/retry discipline as `append-learning-log.sh`) right after computing those numbers, in addition to the existing Telegram notification.

  **Rollup mechanics:** `scripts/rollup_performance_summaries.py` is pure logic (no git) — given the file's current contents, it cascades weekly→monthly→quarterly→half-yearly→yearly in one pass by real calendar boundaries (never a partial/still-open period), recomputing each rolled-up row's blended CPO/ROAS from the *summed* spend/orders/order-value — never averaged from the sub-periods' own blended figures, which would be a subtle but real math error. It's idempotent (a no-op once nothing is due), which is what makes `scripts/run-performance-rollup.sh` (the git-safe wrapper, called once a month from `prompts/monthly-strategic-review.md`) safe to simply re-fetch/re-rebase/recompute-from-scratch/retry on a push conflict, rather than needing real conflict resolution on what is a file *rewrite* (add + remove), not a pure append. Verified end-to-end against a real isolated git remote (not just unit-tested) before shipping — a run of 4 weekly entries for a fully-past month correctly produced one monthly row with correctly-summed totals and recomputed blended figures, and a second run against the resulting state correctly did nothing.

  **No historical backfill** — the five numbers from before 2026-09-05 only ever existed as Telegram text and prose learning-log entries, never in a structured form, so there was nothing to reconstruct; this starts capturing from the next weekly/monthly review going forward.
