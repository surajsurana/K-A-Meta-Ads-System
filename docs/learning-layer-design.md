...
- **Rollup/archival for numeric performance history — proposed 2026-09-05, not yet built.** Suraj's proposal for when this becomes worth doing: weekly performance summaries (spend/orders/blended CPO/ROAS) roll up into monthly once 4 weeks exist, monthly into quarterly once 3 months exist, and so on through half-yearly/yearly — bounding total volume indefinitely (a 10-year-old account would only ever hold ~9 yearly summaries plus the current year's finer-grained ones). Important distinction agreed on: this applies to **plain numeric aggregates**, which compress with no real loss — it does *not* apply to the narrative learning-log entries above (decisions, experiments, overrides), where the specific reasoning is the entire point and a rolled-up summary would destroy exactly the detail this log exists to preserve. If this is built, narrative entries get **archived** (moved out of the actively-scanned file, never deleted) rather than rolled up, so nothing is ever unrecoverable.
