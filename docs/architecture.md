...
## 7a. Pending: Master & Product Sales Intelligence (blocked on Stitchflow data — added 2026-08-21, not built)

**Status: documented dependency only. No agent instructions, prompts, tags, or learning-log conventions have been added for this yet — do not attempt this analysis, and do not invent, estimate, or infer master, capacity, or availability data in its place.** This differs from §7's "deliberately not built" items: those are scope choices this system could build today but chooses not to; this one is **blocked on data that doesn't exist yet in Stitchflow**, not a scope decision. Revisit and design properly once Stitchflow actually supports assigning products to masters and tracking master status/capacity — this entry is the record that the intent exists, not a spec to start implementing early.

**The eventual objective, once unblocked:** primarily commercial, not just operational — which masters' products actually sell better and contribute more, not simply who's available to produce. Where data supports it, the future system should study: orders/revenue/AOV/conversion by master; which masters' products see the most demand; which specific products perform best/worst within a master's own portfolio; whether particular masters consistently produce stronger-selling products; product/category/creative patterns associated with stronger-selling masters; whether a product's performance holds up when reassigned to a different master (where history allows the comparison); and portfolio performance overall, so advertising can prioritize what's commercially proven, not just what's newest or best-liked internally.

**Capacity is explicitly secondary**, layered on top of the commercial read once both exist: combine demand with current master availability so the system can avoid aggressively pushing a full/unavailable/on-leave/discontinued master's products while favoring commercially strong products a currently-available master can still produce.

**The relationship between products and masters must be modeled as flexible and time-based from the start, not a permanent one-product/one-master assumption:** a product may move from one master to another over its life, and some products may be producible by more than one master. Historical analysis must attribute each period to whoever was actually responsible for it *at that time* — a master reassignment must never retroactively rewrite who got credit for a product's past performance.

**Ownership, matching the existing pattern exactly (no new agent, no change to today's roster):** performance-analyst would measure the commercial evidence (by master, by product, by portfolio); campaign-strategist would decide whether advertising should prioritize, maintain, reduce, test, or hold promotion of a product/portfolio, weighing sales performance against production availability — the same measure/decide split already used for Geographic & Demographic Intelligence (§3c) and every other strategic domain in this system.

**Unblocking condition:** Stitchflow must provide (a) product-to-master assignment with real history (not just current-state, given the flexible/reassignable relationship above), and (b) master status/capacity/availability tracking. When that ships, this section should be replaced with a real §3-lettered policy subsection (following the §3a/§3b/§3c pattern) and the corresponding agent-file updates — not built incrementally against partial/assumed data before then.
