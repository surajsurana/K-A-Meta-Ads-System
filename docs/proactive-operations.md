...
- **K&A Ops Console dashboard** (added 2026-09-11, made genuinely live 2026-09-13 — real user feedback: "is it not a live dashboard?") — `live_server.py` (stdlib-only, no pip install) in `~/ka-meta-ads-dashboard`, supervised the same way: `* * * * * scripts/watchdog-dashboard-server.sh` (checks `pgrep -f "[l]ive_server.py"`, bracketed so the pattern never self-matches the watchdog's own invocation — a real incident, see the script's comment). Confirmed externally reachable at `http://168.144.66.161:8090/` with no firewall rule or root needed on this droplet. On each request it serves a **15-minute cache** of the page rebuilt from `template.html` (placeholder tokens like `__ROAS__`) with real, live values substituted in: active-campaign count and total daily budget from Meta (`GET /act_.../campaigns` and `/adsets`), the account's total daily budget ceiling parsed fresh from this doc's own §3a line (never hardcoded), spend-by-country/adset from Meta `/insights` (`date_preset=last_7d`), real order value in the same trailing-7-day window from Stitchflow's `list_orders` (summed client-side per that tool's own guidance, cancelled orders excluded), blended ROAS computed from those two, and the Telegram-approvals-pending count read straight from `~/ka-meta-ads/telegram_approvals_state.json` (status `"pending"` specifically — a tap genuinely still awaiting Suraj, not a resolved/held one). Stitchflow's MCP endpoint (`https://stitchflow.in/api/mcp`, bearer token in `.mcp.json`) is called directly over plain JSON-RPC/HTTP from this stdlib script — confirmed live 2026-09-13 that it needs no session handshake and no MCP client library, just a POST with the `Authorization` header, same as any other REST call. The Claude-hosted artifact version stays a hand-refreshed snapshot (republished whenever a session touches the dashboard) since a static claude.ai artifact has no way to run a backend — only the droplet copy is truly live. `ad-thumbs/` still holds locally-stored copies of the ad creative thumbnails specifically because Meta's own CDN URLs for them expire — never link the page directly to a live `scontent.fbom3-*.fna.fbcdn.net` URL. `template.html` and `live_server.py` are deployed by hand via `scp`, same as `index.html` always was — neither is committed to this git repo.
...
**The weekly and monthly reviews' notifications are the exception to "quiet = silent" (added 2026-08-23, user request):** both always fire, every cycle, and their first lines always state the same five numbers explicitly — total spend, number of orders, total order value, blended cost-per-order, and blended ROAS (monthly total instead of weekly total for the monthly one, previous calendar month) — never silently omitted, even on an otherwise-quiet cycle with nothing else to report. Both use the same fixed template: a `📊 ... Report` header, the five numbers each on their own line, then a blank line before any other findings — plain English, no jargon, no Meta object IDs, no Markdown formatting characters relied on for structure (same reason as §8a's approval messages — Telegram's parser breaks on unescaped `_`/`*` in real plan text). See `prompts/weekly-review.md` and `prompts/monthly-strategic-review.md` for the exact requirement and template. This is deliberately different from the daily heartbeat (which stays silent on a quiet day) — the user wants a standing spend/returns snapshot every week and every month regardless of whether anything else happened.
...
- **`scripts/send-telegram-approval.sh <plan-id>`** — called by an agent once a plan is genuinely ready for approval. Validates the plan exists in `knowledge/learning-log.jsonl` and is `type:decision` (refuses otherwise — never sends an approval request for a plan that doesn't exist or isn't actually a decision). Sends the plan's `telegram_summary` field verbatim if present (added 2026-08-23, user feedback — a short, plain-English, agent-written, pre-formatted-with-line-breaks version naming the real campaign/ad set/ad, not the technical `summary` field's dense prose; see `docs/learning-layer-design.md`), falling back to a truncated `summary` snippet for older entries that predate this convention. Inline keyboard: `✅ APPROVE` / `❌ REJECT` / `🕒 HOLD`, `callback_data` = `A:<plan-id>` / `R:<plan-id>` / `H:<plan-id>`. Records the pending state via the listener's `--record-sent` mode.
- **`scripts/telegram_approval_listener.py`** — a single long-lived process, long-polling `getUpdates`. State lives in `~/ka-meta-ads/telegram_approvals_state.json`, outside the repo (same reasoning as logs, §7), one JSON object keyed by plan id/check id, every read-modify-write wrapped in an `fcntl.flock` so a rapid double-tap can only ever act once (the second callback sees `status != "pending"` and is told "already <status>", nothing more happens). Requests older than 48h (`STALE_AFTER_HOURS`) are marked `expired` on the next tap and never executed. Every callback is checked against the `chat_id` recorded when the request was sent *and* the incoming message's own chat — either mismatch is refused with "Not authorized," and logged. **The execution-plan path (`A:`/`R:`/`H:` callbacks, this section) still only ever processes `callback_query` events — never a free-text message — which is what makes "no arbitrary message can trigger a Meta/Instagram write" hold structurally, not just by convention; this has not changed.** As of 2026-09-04 the listener also handles a second, narrower, structurally distinct callback family (`PY:`/`PN:` — product-identification confirmation, `docs/architecture.md` §3d) that *does* read a plain-text reply, but only ever as the answer to one specific pending "which product is this" question (matched via Telegram's `reply_to_message.message_id`, never inferred), and the only thing it can ever write is a product-identity record — never a Meta/Instagram/Shopify call, never an execution dispatch. The invariant this paragraph states is about the execution-plan path specifically; it still holds without exception there.
  ...
  - **HOLD** → lightweight `type:observation` entry, message edited to show held, terminal for *that specific button instance* (matches the requirement that an already-acted-on button can't be reused) — but not terminal for the plan itself. **Automatically re-asked weekly, after a light re-check, not a blind resend (added 2026-08-24, user request):** the observation entry carries a `follow_up` 7 days out; the weekly review's held-plan sweep (`prompts/weekly-review.md` step 0) checks whether it's already resolved (a later `type:override`/`type:change`, or a `superseded`/`dropped`-tagged observation), and if not, dispatches performance-analyst for a quick fresh pull of just that object's current state before deciding what to do — a week is long enough for a plan's reasoning to go stale even if not literally executed yet. If the fresh check still holds, resends the same plan with fresh buttons ("Hold items — still current"). If it doesn't hold anymore, it does **not** resend the stale plan — it logs what changed, routes through campaign-strategist to decide whether a fresh plan should replace it (`supersedes` the old one) or the whole thing should be dropped, and surfaces that plainly ("Hold items — needs a fresh look") instead of a stale button. Holding a still-current or freshly-superseded plan again creates a new observation entry with its own fresh 7-day `follow_up`, so this repeats automatically every week until the user actually approves or rejects it — a hold is a "not now," never a silent "never," and never a stale rubber-stamp either.
...
**Real (non-`TEST-`) execution is enabled as of 2026-08-21**, gated behind `TELEGRAM_APPROVAL_REAL_EXECUTION=true` in `telegram_config.txt` on the droplet (`telegram_config.txt.example` still ships `false` as the safe default for any fresh clone/redeploy — this is a live-config-only flip, not a code default change). While the switch is unset/false, `dispatch_execution()` records the tap and reports back "approved but not executed — gated" without dispatching any `claude -p` session at all, for exactly the situation where someone wants this feature built and tested without live execution yet turned on. It was left off by default while the path raised an open architectural question — see `docs/architecture.md` §3 guardrail 1's 2026-08-21 resolution note for the reasoning: an independently-authenticated Telegram tap (Telegram's own callback signature plus this project's `chat_id` check, both in code, never asserted by an agent) is a second legitimate direct-approval channel, not the relayed-consent pattern guardrail 1 blocks. The switch itself remains a deliberate code-level gate, not a prompt instruction to the dispatched session — flipping it back to `false` on the droplet immediately and unconditionally stops live execution via this path again, regardless of model behavior.

## 8a-i. Feed-vs-Story repost choice — two plans, one message, two APPROVE buttons (added 2026-09-14, real user request)

**The gap:** social-community-manager was unilaterally picking Feed or Story for a UGC/content repost and sending a single ordinary §8a approval — Suraj could only approve or reject that one choice, never redirect to the other destination without a manual back-and-forth. Real user framing: "I want an option to post on story or feed for such things... sometimes I want to repost but on story and not in my feed."

**The fix:** for any repost where both destinations are genuinely plausible, social-community-manager now writes **two sibling `type:decision` entries** — identical `image_url`/`video_url` and caption, one targeting Feed, one targeting Story — each carrying the other's id in a `"paired_plan_id"` field. These are sent together via **`scripts/send-telegram-approval-repost-choice.sh <feed-plan-id> <story-plan-id>`**, a new sibling script to `send-telegram-approval.sh` (refuses to send unless the two plans' `paired_plan_id` fields genuinely point at each other — never sends two unrelated plans as if they were an either/or choice). One Telegram message, one destination-neutral caption (built by stripping each plan's own `telegram_summary` down to its shared content description), four buttons:

- `✅ Feed` / `✅ Story` — each is just that sibling's own ordinary `A:<plan-id>` callback. No special-casing needed in `handle_callback_query` at all; tapping either runs the exact same single-plan approve path §8a already describes.
- `❌ Reject` / `🕒 Hold` — new `RB:<feed-id>|<story-id>` / `HB:<feed-id>|<story-id>` callback prefixes (handled by the new `handle_paired_callback_query`), which resolve **both** sibling plans in one tap — the point being that rejecting/holding "neither" shouldn't leave the untapped destination's plan dangling pending forever with a dead button next week.

**Race-safety, not just UI convenience:** Telegram's own keyboard-clearing-after-edit isn't a hard guarantee against both `A:` buttons firing before either edit lands — so the real safety net is inside `dispatch_execution`'s prompt (step 2a): before posting, the dispatched session also checks whether its plan's `paired_plan_id` has *already* executed: if so, it stops, posts nothing, and reports `ALREADY_EXECUTED` naming which destination actually went out. Same content can never post twice under this mechanism, regardless of tap timing.

Skip the pairing (send a single ordinary §8a plan) only when the content structurally isn't a Feed/Story choice — a Reel, or something social-community-manager judges is Story-only material (too casual for a permanent Feed placement) — stating that reasoning in the single plan's `telegram_summary` so Suraj can still redirect via Reject if he disagrees.

## 8b. Yes/no confirmation questions — always buttons, never a plain-text question (added 2026-08-30, real gap)

**Never send a plain Telegram notification asking the user a yes/no question and expect a typed reply to close the loop — outside the one narrow, structurally distinct exception in §3d (product-identification correction, added 2026-09-04), the listener only processes `callback_query` events, by deliberate design. A typed reply to anything else in the chat is invisible to every part of this system; nothing reads it back.** This isn't a corner case worth improvising around — it happened live: campaign-strategist found an unexplained ad-set pause, correctly decided it needed the user's direct confirmation, and had marketing-lead send a plain-text question ("did you pause this yourself?"). The user replied in Telegram, in good faith, believing that answered it. The next day's heartbeat asked the identical question again, because nothing had captured the reply — the user had to answer a second time, directly in conversation, before it actually closed.

**The fix: frame the confirmation as a `type:decision` entry and send it through the exact same `send-telegram-approval.sh` mechanism as any other plan** — reusing the proven, already-working button infrastructure rather than inventing free-text parsing (which guardrail 1 and §8a's own design already treat as a solved-not-to-be-reopened question). The "exact plan" content is just unusual in one way: it's confirmation-only, no Meta/Instagram write involved, so the dispatched session logs a fact instead of executing a change:

- **`telegram_summary`** states the question plainly and spells out what each button means, e.g.: *"Did you pause this yourself? Approve = yes, this was me, close it out. Reject = no, I didn't do this — investigate. Hold = not sure yet, ask me later."*
- **APPROVE** → the plan instructs: log a `type:human_knowledge` entry (not `type:change` — nothing on Meta changed) recording the confirmed fact, `linked_to` the original finding. The existing marketing-lead execution-protocol dispatch already follows a plan's own specific instructions over its generic default (proven by the `TEST-`/smoke-test plans that already log non-`type:change` entries correctly) — no code change needed, just write the plan to say so explicitly.
- **REJECT** means something different here than a normal declined plan — say so in the plan text: it means "no, I did not do this," and the resulting log entry should read as a genuine access-anomaly escalation (`type:observation`, flagged for investigation), not the standard "declined, no changes made" wording that fits a normal execution plan.
- **HOLD** behaves exactly as normal (§8a) — appropriate if the user genuinely isn't sure yet.

Whichever agent first needs to ask the user to confirm a real-world fact (not "should we do X," but "did you do X") should use this pattern — see `.claude/agents/campaign-strategist.md`'s anomaly-handling guidance for the pointer back here.
