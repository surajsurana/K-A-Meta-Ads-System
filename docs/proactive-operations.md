...
- `--permission-mode`/`--allowedTools` scoped to read-only Meta `GET` calls, the learning-log script, and the Telegram alert path — exact flag syntax in the wrapper scripts is indicative and must be verified against the installed CLI version during deployment (§10), not treated as gospel.
...
**Primary: Telegram.** Two independent paths, both real and built:
1. **In-session** — the agent itself, when it reaches a validated plan awaiting approval, sends the notification via `curl` to the Telegram Bot API, using `telegram_config.txt` (gitignored, local to the repo working directory — same pattern as `meta_token.txt.txt`; `telegram_config.txt.example` documents the required shape). This is what carries the plan's actual content and the relevant learning-log entry id(s) so the next interactive VS Code session can find it immediately (e.g. via `grep <id> knowledge/learning-log.jsonl`).
2. **Wrapper-script-level failure alert** (added this pass, a real gap in the original design) — sent directly by `scripts/_run-common.sh`, independent of whether the Claude session ran at all. If the sync fails or the headless CLI process itself exits non-zero, this path still fires, because it doesn't depend on the agent having gotten far enough to notify itself.
...
5. Re-register the cron jobs from §6.
