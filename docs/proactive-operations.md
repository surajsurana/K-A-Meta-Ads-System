...
- **K&A Ops Console dashboard** (added 2026-09-11) — a plain `python3 -m http.server 8090 --directory ~/ka-meta-ads-dashboard` process, supervised the same way: `* * * * * scripts/watchdog-dashboard-server.sh`. Confirmed externally reachable at `http://168.144.66.161:8090/` with no firewall rule or root needed on this droplet. Static snapshot, same content as the Claude-hosted artifact version — refreshed by hand (regenerate `index.html`, `scp` it and any changed `ad-thumbs/*.jpg` over, no server restart needed) whenever the artifact is updated, not on an automatic schedule. `ad-thumbs/` holds locally-stored copies of the ad creative thumbnails specifically because Meta's own CDN URLs for them expire — never link the page directly to a live `scontent.fbom3-*.fna.fbcdn.net` URL.
